...
namespace mu
{
  ...
  */
  ...
  struct MathImpl
  {
    ...
    static T ATan2(T v1, T v2) { return std::atan2(v1, v2); }
    static T Sinh(T v)  { return std::sinh(v); }
    static T Cosh(T v)  { return std::cosh(v); }
    static T Tanh(T v)  { return std::tanh(v); }
    static T ASinh(T v) { return std::log(v + sqrt(v * v + 1)); }
    static T ACosh(T v) { return std::log(v + sqrt(v * v - 1)); }
    static T ATanh(T v) { return ((T)0.5 * std::log((1 + v) / (1 - v))); }
    static T Log(T v)   { return std::log(v); }
    static T Log2(T v)  { return std::log(v)/std::log((T)2); } // Logarithm base 2
    static T Log10(T v) { return std::log10(v); }         // Logarithm base 10
    static T Exp(T v)   { return std::exp(v);   }
    static T Abs(T v)   { return (v>=0) ? v : -v; }
    static T Sqrt(T v)  { return std::sqrt(v); }
    static T Rint(T v)  { return std::floor(v + (T)0.5); }
